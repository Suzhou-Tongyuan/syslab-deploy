...
static void AllowRegisterThreads()
{
    GC_allow_register_threads();
}

inline ManagedThreadRegistered::ManagedThreadRegistered()
{
    /* bdwgc中定义了：#define GC_DUPLICATE 1, 表示在当前线程中重复调用了 GC_register_my_thread */
    const int gcDuplicateCode = 1;
    GC_stack_base sb;
    GC_get_stack_base(&sb);
    int register_code = GC_register_my_thread(&sb);
    isDuplicateRegister = (register_code == gcDuplicateCode);
}
...
inline ManagedThreadRegistered::~ManagedThreadRegistered()
{
    ...
    {
        // 只有非duplicate register的析构才调用GC_unregister_my_thread
