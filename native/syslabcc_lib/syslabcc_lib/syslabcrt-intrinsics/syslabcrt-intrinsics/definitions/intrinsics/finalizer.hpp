...
static unused_t _sj_gc_add_func_finalizer(FuncRetT (*func)(ptr_t<_sj_thread_context>, FuncSelfT, ptr_t<ObjT>),
                                          FuncSelfT funcSelf, ptr_t<ObjT> obj)
{
    using F_t = void(void *);
    GC::RegisterFinalizer(obj.Raw, [=](void *obj_) -> void {
        _sj_managed_thread_context _managed_thread_context;
        func(_managed_thread_context.context, funcSelf, ptr_t<ObjT>(obj_));
        return;
    });
    return unused_t();
}
